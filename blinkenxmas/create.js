function initCreateForm(form) {
    ...
    form.elements['name'].addEventListener(
        'input', (evt) => createOrUpdate(form));
    form.elements['animation'].addEventListener(
        'change', (evt) => setupCreateForm(form));
...
function setupCreateForm(form) {
    ...
    if (animation) {
        getAnimations().then((data) => {
            ...
            // Create new elements for the selected animation before the
